...
Expr* comp(Tag tag, Expr* a) { return comp(tag, &a, 1); }
...
Expr* comp(Tag tag, Vec<Expr*>& v) { return comp(tag, v.data, v.n); }
...
bool eq(Comp* a, Comp* b) { return eq(a->tag, a->v, a->n, b); }

size_t hash(Tag tag, Expr** a, size_t n) {
	// TODO: hashCombine?
	return fnv(a, n * sizeof(void*));
}
size_t hash(Comp* a) { return hash(a->tag, a->v, a->n); }
...
Expr* compi(Tag tag, Vec<Expr*>& v) { return comps.intern(tag, v.data, v.n); }
